...
int main(int argc, char **argv)
{
    ...
    if (decode)
    {
        uint8_t bin[decode];
        size_t ssz = decode;
        if (!base58_to_bin(bin, &ssz, r.s, rt))
            return 2;
        if (b58c)
        {
            int chk = base58_check(bin, decode, r.s, rt);
            if (chk < 0)
                return chk;
    ...
    else
    {
        ...
        bool rv;
        if (b58c)
            rv = rt && base58_check_encode(s, &ssz, r.b[0], &r.b[1], rt - 1);
        else
            rv = base58_encode(s, &ssz, r.b, rt);
        if (!rv)
            return 2;
        puts(s);
    }
